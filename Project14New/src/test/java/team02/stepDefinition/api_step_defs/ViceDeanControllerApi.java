...
public class ViceDeanControllerApi {
    ...
    public void vice_dean_save_icin_1payload_duzenlenir() {
        payload = new ViceDeanPostPojo("1990-01-01",
                "manassas",
                "MALE",
                "celal",
                "Celal7186.",
    ...
    public void kayitli_vice_dean_versinin_ıd_numarasini_alma() {
        spec.pathParams("first", "vicedean", "second", "getAll");
        Response response = given(spec).when().get("{first}/{second}");

        JsonPath json = response.jsonPath();
        userId = json.getInt("find{it.username=='celaltaskirdi'}.userId");
    }
    @Given("Vice Dean getViceDeanById icin URL duzenlenir")
    ...
    public void vice_dean_get_Vicedean_by_ıd_icin_get_request_gonderilir_ve_response_alinir() {
        response = given(spec).when().get("{first}/{second}/{third}");
        actualData = response.as(ViceDeanPostResponsePojo.class);
    }
    @Then("Vice Dean getViceDeanById icin gelen Response Body dogrulanir")
    public void vice_dean_get_by_ıd_icin_gelen_response_body_dogrulanir() {
