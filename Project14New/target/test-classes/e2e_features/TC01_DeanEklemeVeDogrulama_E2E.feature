...
Feature: Dean Ekleme, Dogrulama ve Silme
  ...
  @UI
  Scenario: Admin Web Sitesinde Dean Ekleyebilmeli (UI Testi)
    ...
    And DeanBirthPlace kutusuna "istanbul" girer
    And DeanGender "Female" secenegini girer
    And DeanDateOfBirth kutusuna "05051975" girer
    And DeanPhone kutusuna "355-275-7695" girer
    And DeanSSN kutusuna "538-76-7867" girer
    And DeanUserName kutusuna "ayseyilmaz5" girer
    And DeanPassword kutusuna sifre girer
    And DeanSubmit butonuna tiklar
    Then Dean kaydi yapildigi dogrulanir

  Scenario: Olusturulan Dean hesap bilgisini alma testi (API Testi)
  ...
  Scenario: Kayitli Dean Bilgisi Dogrulama Testi9 (DB Testi)
  ...
  Scenario: Kayitli Dean hesabini silme testi (API Testi)
    Given "Admin" yetkisi ile giris yapilir
